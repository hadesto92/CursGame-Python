...
def scale(self, old_size: tuple, new_size: tuple, is_pos_scale = False):
    #Zapisanie starego położenia obrazka
    old_x = self.x
    old_y = self.y
    ...
    #Zmiana skalowania zakotwiczenia
    try:
        self.anchor = (self.anchor[0]*multipler_size[0], self.anchor[1]*multipler_size[1])
    except TypeError:
        pass
...
def scale_to(object: list, old, new, is_pos_scale = False):
    for obj in object:
        scale(obj, old, new, is_pos_scale)

#Funkcja która obraca obrazek lustrzanie
def mirror_flip(self, flip_x, flip_y):
    #Aby utworzyć funkcję w bibliotece actor.py dodałem zmienne: _fliped_x = _fliped_y = False, dzięki temu wiem
    # czy obrazek jest zwórcony w lewo, prawo, góra lub dół
    if flip_x:
